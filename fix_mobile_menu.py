...
for f in glob.glob('site-a/**/index.html', recursive=True):
    ...
    # Skip if already has mm-nav (already correct)
    ...
    old_pattern = re.compile(
        r'<div class="mobile-menu-overlay".*?</nav>\s*</div>',
        re.DOTALL
    )
    ...
    # Add mobile menu CSS if mm-nav styles not present
...
print(f"Already correct: {skipped_already} files")
if no_match:
    print(f"No match ({len(no_match)}): {no_match[:5]}")
